...
def parse_args():
    ...
    parser.add_argument('data_type', help=
        'The datatype corresponding to the the app. This id is shared between the ' +
        'staging service and the narrative, for example sra_reads')
    parser.add_argument('--tsv', action='store_true', help=
        'Create a TSV file rather than a CSV file (the default)')
    parser.add_argument('--env', choices=['prod', 'appdev', 'next', 'ci'], default='prod', help=
        'The KBase environment to query, default prod')
    parser.add_argument('--print-spec', action='store_true', help=
        'Print the input specification for the app to stderr')
