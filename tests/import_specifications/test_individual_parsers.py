...
def _xsv_parse_success_with_numeric_headers(temp_dir: Path, sep: str, parser: Callable[[Path], ParseResults]):
    s = sep
    input_ = temp_dir / str(uuid.uuid4())
    with open(input_, "w") as test_file:
        test_file.writelines(
...
def test_xsv_parse_fail_no_file(temp_dir: Path):
    ...
    assert res == ParseResults(errors=tuple([Error(ErrorType.FILE_NOT_FOUND, source_1=SpecificationSource(input_))]))
...
def _excel_parse_fail(test_file: str, message: str = None, errors: list[Error] = None, print_res=False):
