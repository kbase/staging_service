...
def _dts_manifest_parse_fail(input_file: Path, errors: list[Error]):
    ...
    error_infos - a list of error info dicts, expected to be in the order generated by the call to
        parse_dts_manifest. Expected keys are 'type' (will fail without) and 'text' which
        defaults to None.

    This auto-generates the tuple of Error objects for testing against a failed parse_dts_manifest
    call.
    """
